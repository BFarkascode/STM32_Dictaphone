 ...
 *  Program version: 1.0
 *  Source file: SDcard_diskio.c
 *  Change history:
 */

#include "SDcard_diskio.h" /* Declarations of disk functions */
...
DRESULT disk_ioctl(
  ...
  switch (cmd) {
    ...
    case CTRL_SYNC: { //this is a timeout step: we do a while loop until we get 0xFF as an SPI Rx byte AND we haven't run out of a timer_cnt
      ...
      do {  //do this loop while the incoming SPI Rx byte is NOT 0xFF AND our timer still hasn't reached 0
        ...
        SPI1_Master_SD_read(&Rx_byte_sync[0], 1);  //we read out a byte
  ...
  SPI1_Master_SD_read(&Rx_byte_dummy[0], 1);  //we read out a byte
